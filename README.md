...
- **Умный поиск с debounce**: ввод в поле поиска не дергает API на каждый символ, запрос уходит с небольшой задержкой через `lodash.debounce`
...
- **Корректные пути к картинкам на GitHub Pages** — работа с относительными путями `imageUrl` из бэкенда без привязки к абсолютному `/`
